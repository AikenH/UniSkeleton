"""
@AikenHong 2021 
Using this file to Intergrate the Ntxent Loss for the SimCLR
And we Using the Memory Bank From MoCo to increase the negative pair in it.
...
"""
import torch
from loss.memory_bank import MemoryBankModule
...
class NTXentLoss(MemoryBankModule):
    ...
    Examples:
        ...
        >>> loss = loss_fn(output)
    """
    def __init__(self, temperature=0.5, memory_bank_size=0):
        super(NTXentLoss, self).__init__(memory_bank_size)
        self.temperature = temperature
        self.cross_entropy = torch.nn.CrossEntropyLoss(reduction="mean")
        self.eps = 1e-8

        assert abs(self.temperature) >= self.eps, \
    ...
    def forward(self, out0: torch.Tensor, out1: torch.Tensor):
        ...
        # get negative pair from the memory bank
        out1, negatives = \
        ...
        # calculate the cosine similarity for the pos and neg
        if negatives is not None:
            # use negatives from memory bank(we may need to check the GPU memory here)
            ...
            # and the true lable is always 0 ,so we use zeros
